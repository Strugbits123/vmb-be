...
const getRequestedGifts = async (req, res) => {
    try {
        ...
        const result = await giftService.getRequestedGifts(requesterId, page, limit, sort);
...
const getRecievedGifts = async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 10;
        const sort = req.query.sort || "newest";
        const result = await giftService.getRecievedGifts(receiverEmail, page, limit, sort);
